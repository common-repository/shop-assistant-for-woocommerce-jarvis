    ...
    self.each(function(baseIndex, baseElem) {
        ...
        function setup() {
            ...
            $.each(settings.new_row_layouts, function(j, layout) {
                var btn = $('<a style ="color:#2CC185;width: 19.60%; margin-right:5px;padding:5px;font-size:15px;background-color:#fff;border: 1px solid #3c763d ;" class="btn btn-xs" />')
        ...
        function createRowControls() {
			...
			if (typeof defaultRow== typeof undefined || defaultRow== false) {
			 //if(row.index()>0){	
                createTool(drawer, 'Remove qc-row', '', 'glyphicon-trash', function() {
        ...
        function createDetails(container, cssClasses) {
            ...
            cssClasses.forEach(function(rowClass) {
                var btn = $('<a class="qc-btn qc-btn-xs qc-btn-default" />')
                    .html(rowClass.label)

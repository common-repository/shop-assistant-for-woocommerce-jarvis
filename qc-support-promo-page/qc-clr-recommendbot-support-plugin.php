                    ...
                    foreach ( $argus as $arg ) {
                        ...
                        foreach ( (array) $qcld_plugininstal as $key => $plugin ) {
                            ...
                            if ( ! empty( $author ) ) {
                                ...
                                $author = ' <cite>' . sprintf( __( 'By %s', 'jarvis' ), $author ) . '</cite>';
                            }
                                ...
                                switch ( $status['status'] ) {
                                        ...
                                        if ( $status['url'] ) {
                                            if ( $compatible_php && $compatible_wp ) {
                                                $action_links[] = sprintf(
                                                    ...
                                                    esc_attr( sprintf( esc_html( 'Install %s now', 'jarvis' ), $name ) ),
                                                    esc_attr( $name ),
                                                    esc_html( 'Install Now', 'jarvis' )
                                                );
                                            } else {
                                                $action_links[] = sprintf(
                                                    '<button type="button" class="button button-disabled" disabled="disabled">%s</button>',
                                                    esc_html( 'Cannot Install', 'jarvis' )
                                                );
                                            }
                                        }
                                        break;
                                        ...
                                        if ( $status['url'] ) {
                                            if ( $compatible_php && $compatible_wp ) {
                                                $action_links[] = sprintf(
                                                    ...
                                                    esc_url( $status['url'] ),
                                                    /* translators: %s: Plugin name and version. */
                                                    esc_attr( sprintf( esc_html( 'Update %s now', 'jarvis' ), $name ) ),
                                                    esc_attr( $name ),
                                                    esc_html( 'Update Now', 'jarvis' )
                                                );
                                            } else {
                                                $action_links[] = sprintf(
                                                    '<button type="button" class="button button-disabled" disabled="disabled">%s</button>',
                                                    esc_html( 'Cannot Update', 'jarvis' )
                                                );
                                            }
                                        }
                                        break;
                                    ...
                                    case 'newer_installed':
                                        if ( is_plugin_active( $status['file'] ) ) {
                                            $action_links[] = sprintf(
                                                '<button type="button" class="button button-disabled" disabled="disabled">%s</button>',
                                                esc_html( 'Active', 'jarvis' )
                                            );
                                        } elseif ( current_user_can( 'activate_plugin', $status['file'] ) ) {
                                            $button_text = esc_html( 'Activate', 'jarvis' );
                                            /* translators: %s: Plugin name. */
                                            $button_label = esc_html( 'Activate %s', 'jarvis' );
                                            ...
                                            if ( is_network_admin() ) {
                                                $button_text = __( 'Network Activate', 'jarvis' );
                                                /* translators: %s: Plugin name. */
                                                $button_label = esc_html( 'Network Activate %s', 'jarvis' );
                            ...
                            $action_links[] = sprintf( '%s','<a href="'.esc_url($plugin_live_link).'" target="_blank">' . esc_html('More Details', 'jarvis') . '</a>');
                            /*===show icon ==*/
                            ...
                            <div class="plugin-card plugin-card-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
                                ...
                                if ( ! $compatible_php || ! $compatible_wp ) {
                                    ...
                                    if ( ! $compatible_php && ! $compatible_wp ) {
                                        esc_html_e( 'This plugin doesn&#8217;t work with your versions of WordPress and PHP.', 'jarvis' );
                                        if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
                                            printf(
                                            /* translators: 1: URL to WordPress Updates screen, 2: URL to Update PHP page. */
                                                ' ' . esc_attr( '<a href="%1$s">Please update WordPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
                                            ...
                                            /* translators: %s: URL to WordPress Updates screen. */
                                                ' ' . esc_attr( '<a href="%s">Please update WordPress</a>.' ),
                                                esc_url(self_admin_url( 'update-core.php' ))
                                            );
                                        } elseif ( current_user_can( 'update_php' ) ) {
                                            printf(
                                            /* translators: %s: URL to Update PHP page. */
                                                ' ' . esc_attr( '<a href="%s">Learn more about updating PHP</a>.' ),
                                                esc_url( wp_get_update_php_url() )
                                            );
                                            wp_update_php_annotation( '</p><p><em>', '</em>' );
                                        }
                                    } elseif ( ! $compatible_wp ) {
                                        esc_html_e( 'This plugin doesn&#8217;t work with your version of WordPress.', 'jarvis' );
                                        ...
                                        esc_html_e( 'This plugin doesn&#8217;t work with your version of PHP.', 'jarvis' );
                                ...
                                <div class="plugin-card-top">
                                    ...
                                    <div class="action-links">
                                        <?php
                                        if ( $action_links ) {
                                            echo '<ul class="plugin-action-buttons"><li>' . implode( '</li><li>', esc_attr($action_links) ) . '</li></ul>';
                                        }
                                        ?>
                                    </div>
        							<?php 
                                    ...
                                    <div class="action-pro-links">
                                        ...
                                        if ( !empty( $arg['live_preview'] ) ) { 
                                            echo '<li><a href="'.esc_url( $arg['live_preview'] ).'" target="_blank">' . esc_html('Live Preview', 'jarvis') . '</a></li>';
                                        }
                                        if ( !empty( $arg['update_to_pro'] ) ) { 
                                            echo '<li><a href="'.esc_url( $arg['update_to_pro'] ).'" target="_blank">' . esc_html('Update To Pro', 'jarvis') . '</a></li>';
                                        }
                                        ?>
        								</ul>
                                    </div>

...
if( !class_exists('QcJarvisPluginUpgradeToProNotice') )
{
	class QcJarvisPluginUpgradeToProNotice {
		...
		function func_show_upgrade_link_with_action_links( $links ){
			...
			$links = array_merge( $links, array(
				'<a title="Settings" class="'.esc_attr($this->link_class).'" style="" href="' . esc_url( admin_url('admin.php?page=jarvis') ) . '" target="">' . __( 'Settings', 'jarvis' ) . '</a>'
			) );

			$links = array_merge( $links, array(
				'<a title="Support" class="'.esc_attr($this->link_class).'" style="" href="' . esc_url( 'https://www.quantumcloud.com/resources/free-support/' ) . '" target="_blank">' . esc_html( 'Support', 'jarvis' ) . '</a>'
			) );

			$links = array_merge( $links, array(
				'<a title="'.esc_attr($this->link_text).'" class="'.esc_attr($this->link_class).'" style="font-weight: bold; color: '.esc_attr($this->link_color).';" href="' . esc_url( $this->upgrade_link ) . '" target="'.esc_attr($this->link_target).'">' . esc_html( $this->link_text, 'jarvis' ) . '</a>'
			) );
		...
		function func_show_upgrade_link_with_meta_links( $links, $file )
		{
			...
			if ( strpos( $file, "$this->plugin_main_file" ) !== false ) {
				...
				$new_links = array(
					'<a class="'.esc_attr($this->link_class).'" style="font-weight: bold; color: '.esc_attr($this->link_color).';" href="' . esc_url( $this->upgrade_link ) . '" title="'.esc_attr($this->link_text).'" target="'.esc_attr($this->link_target).'">' . esc_html( $this->link_text, 'jarvis' ) . '</a>'
				);
	...
	} //End of class QcJarvisPluginUpgradeToProNotice
...
$instance_ilistf = new QcJarvisPluginUpgradeToProNotice();
...
{ 
	...
	$instance_ilistf->plugin_slug = 'shop-assistant-for-woocommerce-jarvis'; //Plugin Slug. i.e. Folder Name
	$instance_ilistf->plugin_main_file = 'jarvis-woo.php'; //Primary file of the pluign
	$instance_ilistf->plugin_menu_slug = 'jarvis'; //Main Menu Slug

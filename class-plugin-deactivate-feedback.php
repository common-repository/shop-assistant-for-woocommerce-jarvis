...
if( ! class_exists( 'Wp_jarvis_assistant_Usage_Feedback') ) {
	
	class Wp_jarvis_assistant_Usage_Feedback {
		...
		public function get_data() {
			...
			if( empty( $plugin ) ) {
				...
				$body['message'] .= __( 'We can\'t detect any plugin information. This is most probably because you have not included the code in the plugin main file.', 'jarvis' );
				$body['status'] = __( 'Data not found', 'jarvis' ); // Never translated
			} else {
		...
		public function form_default_text() {
			$form = array();
			$form['heading'] = __( 'Sorry to see you go', 'jarvis' );
			$form['body'] = __( 'Before you deactivate the plugin, would you quickly give us your reason for doing so? Only the option you selected in this form will be emailed to us.', 'jarvis' );
			$form['options'] = array(
				__( 'Found a Bug', 'jarvis' ),
				__( 'Need More Features', 'jarvis' ),
				__( 'Deactivating Temporarily', 'jarvis' ),

			);
			$form['details'] = __( 'Please provide some details so we can improve the plugin for you (optional)', 'jarvis' );
			return $form;
		}
		...
		public function goodbye_ajax() {
			...
			$html = '<div class="wpb-goodbye-form-head"><strong>' . esc_html( $form['heading'], 'jarvis' ) . '</strong></div>';
			$html .= '<div class="wpb-goodbye-form-body"><p>' . esc_html( $form['body'], 'jarvis' ) . '</p>';
			if( is_array( $form['options'] ) ) {
				...
				$html .= '</p><label for="wpb-goodbye-reasons">' . esc_html( $form['details'], 'jarvis' ) .'</label><textarea name="wpb-goodbye-reasons" id="wpb-goodbye-reasons" rows="2" style="width:100%"></textarea>';
				$html .= '</div><!-- .wpb-goodbye-options -->';
			}
			$html .= '</div><!-- .wpb-goodbye-form-body -->';
			$html .= '<p class="deactivating-spinner"><span class="spinner"></span> ' . __( 'Submitting form', 'jarvis' ) . '</p>';
			?>
			...
			<script>
				jQuery(document).ready(function($){
					$("#wpb-goodbye-link-<?php echo esc_attr( $this->plugin_name ); ?>").on("click",function(){
						...
						$("#wpb-goodbye-form-<?php echo esc_attr( $this->plugin_name ); ?>").html( '<?php echo esc_attr($html); ?>' + '<div class="wpb-goodbye-form-footer"><p><a id="wpb-submit-form" class="button primary" href="#">Submit and Deactivate</a>&nbsp;<a class="secondary button" href="'+url+'">Just Deactivate</a></p></div>');
						$('#wpb-submit-form').on('click', function(e){
							...
							var data = {
								...
								'values': values,
								'details': details,
								'security': "<?php echo esc_attr(wp_create_nonce( 'wpbot_goodbye_form' )); ?>",
								'dataType': "json"
							}
							$.post(
								ajaxurl,
